...
function TrackingBadge({ runId }: { runId: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 font-medium flex-shrink-0">
      <Send className="w-2.5 h-2.5" /> PostulÃ©
    </span>
  );
}
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h1 className="text-3xl font-extrabold tracking-tight text-foreground flex items-center gap-3">
              <Clock className="w-7 h-7 text-primary" /> Historique
            </h1>
            <p className="text-muted-foreground text-sm">Retrouve tous tes CV generes et leur statut.</p>
          </div>
          <Link href="/tailor">
            <Button data-testid="button-new-tailor" className="rounded-xl shadow-sm shadow-primary/20">
              <WandSparkles className="w-4 h-4 mr-2" /> Nouveau tailoring
            </Button>
          </Link>
        </div>
          ...
          <Card className="border-dashed border-2 bg-transparent shadow-none">
            <CardContent className="flex flex-col items-center justify-center py-16 gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Ton historique apparaitra ici apres ton premier tailoring.</p>
              </div>
              <Link href="/tailor">
                <Button variant="outline" className="rounded-xl mt-2" data-testid="button-start-tailoring">
                  <WandSparkles className="w-4 h-4 mr-2" /> Lancer un tailoring
                </Button>
              </Link>
            </CardContent>
          </Card>
                    ...
                    <CardContent className="p-4 flex items-center gap-4">
                      <div className="flex-1 min-w-0 space-y-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {report?.jobCompany && (
                            <span className="text-xs text-muted-foreground truncate">â€” {report.jobCompany}</span>
                          )}
                        </div>
                        <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {displayScore != null && (
                          <div className="space-y-1">
                            ...
                            <ConfidenceBar value={displayScore} />
                          </div>
                        )}
                      </div>
