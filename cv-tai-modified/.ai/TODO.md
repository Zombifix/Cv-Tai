...
## Next
- Verifier que `/api/check-match` renvoie maintenant `viability`, `shouldWarn`, `warningMessage`, `precheckMode` et `jobInput.scrapeQuality`, et que le formulaire Tailor ne bloque plus absurdement des offres design viables.
- Verifier que le pre-check ne sort plus de warning dur sur des annonces design proches, mais continue d'alerter sur les vrais mismatches forts.
- Verifier que les annonces scrapees `uncertain` affichent bien un badge/notice de qualite de scrape dans Tailor et Result.
- Verifier qu'une annonce avec encodage casse est nettoyee plus tot par `repairScrapeEncoding` et dedupee avant scoring.
- Verifier que le mode `polished` reste non regressif sur les deux axes visibles (`Fit offre` et `confidence` legacy) face au baseline `original`.
...
- Le build local a retrouve `npm run check`; conserver `target: ES2020` dans `tsconfig.json` pour eviter les faux positifs TypeScript sur `Set` / `Map`.
