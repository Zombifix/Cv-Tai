...
Reponds UNIQUEMENT en JSON :
{
  ...
  // ── SETTINGS — import user data from JSON export ──
  app.post("/api/settings/import", requireAuth, async (req, res) => {
    try {
      const userId = uid(req);
      const { profile: importedProfile, experiences: importedExperiences, skills: importedSkills, formations: importedFormations, languages: importedLanguages } = req.body;

      const client = await (await import("./db")).pool.connect();
      try {
        // Wipe existing user data (keep runs/job_posts intact)
        ...
        if (importedProfile) {
          await client.query(
            "INSERT INTO profile (id, user_id, name, title, summary, target_role, updated_at) VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, NOW())",
            [userId, importedProfile.name || "", importedProfile.title || "", importedProfile.summary || null, importedProfile.targetRole || null],
          );
        }

        // Restore experiences + bullets
        let expCount = 0;
        let bulletCount = 0;
        for (const exp of (importedExperiences || [])) {
          const expResult = await client.query(
            "INSERT INTO experiences (id, user_id, title, company, contract_type, start_date, end_date, description, priority, created_at) VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, $8, NOW()) RETURNING id",
            [userId, exp.title || "", exp.company || "", exp.contractType || null, exp.startDate || null, exp.endDate || null, exp.description || null, exp.priority ?? 0],
          );
          const newExpId = expResult.rows[0].id;
          expCount++;
          for (const bullet of (exp.bullets || [])) {
            await client.query(
              "INSERT INTO bullets (id, experience_id, text, priority, tags, created_at) VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW())",
              [newExpId, bullet.text || "", bullet.priority ?? 0, bullet.tags || []],
            );
            bulletCount++;
          }
        }
        ...
        for (const skill of (importedSkills || [])) {
          await client.query(
            "INSERT INTO skills (id, user_id, name, category, level, priority, created_at) VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, NOW()) ON CONFLICT DO NOTHING",
            [userId, skill.name || "", skill.category || null, skill.level ?? null, skill.priority ?? 0],
          );
          skillCount++;
        }

        // Restore formations
        for (const f of (importedFormations || [])) {
          await client.query(
            "INSERT INTO formations (id, user_id, school, degree, year, created_at) VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW())",
            [userId, f.school || "", f.degree || "", f.year || null],
          );
        }

        // Restore languages
        for (const l of (importedLanguages || [])) {
          await client.query(
            "INSERT INTO languages (id, user_id, name, level, created_at) VALUES (gen_random_uuid(), $1, $2, $3, NOW())",
            [userId, l.name || "", l.level || null],
          );
        }

        console.log(`[SETTINGS] Import done for user ${userId}: ${expCount} exps, ${bulletCount} bullets, ${skillCount} skills`);
        res.json({ success: true, experiences: expCount, bullets: bulletCount, skills: skillCount });
      } finally {
        client.release();
      }
    } catch (err: any) {
